...
//    scanf("%d%d",&n,&m);
    clr(a, 0);
    FOR(i,n + 2) FOR(j,m + 2) a[i][j] = 1;
    for(int i = 1; i < n + 1; i++) a[i][1] = 0;
    for(int i = 1; i < m + 1; i++) a[n][i] = 0;
    FOR(i,n + 2) FOR(j,m + 2) val[i][j] = 1000000;
    FOR(i,n) FOR(j,m) scanf("%d",&val[i][j + 2]);
//    cout << endl;
//    FOR(i,n + 2)
//    {
//        FOR(j,m + 2) cout << val[i][j] << "\t" ;cout << endl;
//    }
    n += 2;
    m += 2;
...
void dpblank(int i, int j, int cur)
{
    FOR(k, hm[cur].size)
    {
        ...
        if(k1 + k2 == 0)
        {
            hm[nxt].push(clrbit(hm[cur].state[k], j, j + 1) | (L << mov[j]) | (R << mov[j + 1]), hm[cur].dp[k] + val[i][j]);
            hm[nxt].push(hm[cur].state[k], hm[cur].dp[k]);
        }
        else if(k1 == R && k2 == L) hm[nxt].push(clrbit(hm[cur].state[k], j, j + 1), hm[cur].dp[k] + val[i][j]);
        else if(k1 == L && k2 == R)
        {
            if(i == fx && j == fy) hm[nxt].push(clrbit(hm[cur].state[k], j, j + 1), hm[cur].dp[k] + val[i][j]);
        }
        else if(k1 == L && k2 == L) hm[nxt].push(fll(hm[cur].state[k], j), hm[cur].dp[k] + val[i][j]);
        else if(k1 == R && k2 == R) hm[nxt].push(frr(hm[cur].state[k], j), hm[cur].dp[k] + val[i][j]);
        else
        {
            hm[nxt].push(hm[cur].state[k], hm[cur].dp[k] + val[i][j]);
            hm[nxt].push(clrbit(hm[cur].state[k], j, j + 1) | (k2 << mov[j]) | (k1 << mov[j + 1]), hm[cur].dp[k] + val[i][j]);
        }
    }
}
void shift(int cur)
...
//        {printf("\t");putst(hm[cur].state[k]);puts("");}
    ...
    FOR(i,hm[cur].size) if(hm[cur].state[i] == 0) cmax(ans,hm[cur].dp[i]);
    ans -= 1000000L * (n + m + 1);
    printf("%I64d\n",ans);
}
int main()
