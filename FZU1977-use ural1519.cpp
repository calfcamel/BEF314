...
//    scanf("%d%d",&n,&m);
    ...
    fx = 0;
    fy = 0;
    FOR(i,n) FOR(j,m) if(a[i][j] == 1) {fx = i; fy = j;}
}
int t1,t2;
int k1,k2;
...
void dpblank(int i, int j, int cur)
{
    FOR(k, hm[cur].size)
    {
        ...
        if(k1 + k2 == 0)
        {
            if(a[i][j] == 2) hm[nxt].push(hm[cur].state[k], hm[cur].dp[k]);
            hm[nxt].push(clrbit(hm[cur].state[k], j, j + 1) | (L << mov[j]) | (R << mov[j + 1]), hm[cur].dp[k]);
        }
        else if(k1 == R && k2 == L) hm[nxt].push(clrbit(hm[cur].state[k], j, j + 1), hm[cur].dp[k]);
        else if(k1 == L && k2 == R)
        {
            if((i == fx && j >= fy) || (i > fx)) if(clrbit(hm[cur].state[k], j, j + 1) == 0) ans += hm[cur].dp[k];
        }
