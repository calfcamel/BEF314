...
void init()
{
    ...
    FOR(i,2 * n + 1)
    {
        if(i == 0 || i == 2 * n) scanf("%s",str);
        else if(i & 1)
        {
            scanf("%s",str);
            FOR(j,m - 1) scanf("%d",&_r[i >> 1][j]);
            scanf("%s",str);
        }
        else
        {
            scanf("%s",str);
            FOR(j,m) _d[(i - 1) / 2][j] = str[j * 2 + 1] - '0';
        }
    }
//    cout << "_d:" << endl;
//    FOR(i,n - 1)
//    {
//        FOR(j,m) cout << _d[i][j] << " "; cout << endl;
//    }
//    cout << "_r:" << endl;
//    FOR(i,n)
//    {
//        FOR(j,m - 1) cout << _r[i][j] << " "; cout << endl;
//    }
...
void dpblank(int i, int j, int cur)
{
    FOR(k, hm[cur].size)
    {
        ...
        k1 = getbit(hm[cur].state[k], j);
        k2 = getbit(hm[cur].state[k], j + 1);
        if(k1 + k2 == 0) hm[nxt].push(clrbit(hm[cur].state[k], j, j + 1) | (L << mov[j]) | (R << mov[j + 1]), hm[cur].dp[k] + _r[i][j] + _d[i][j]);
        ...
        {
            hm[nxt].push(hm[cur].state[k], hm[cur].dp[k] + (k1? _d[i][j] : 0) + (k2? _r[i][j] : 0));
            hm[nxt].push(clrbit(hm[cur].state[k], j, j + 1) | (k2 << mov[j]) | (k1 << mov[j + 1]), hm[cur].dp[k] + (k2? _d[i][j] : 0) + (k1? _r[i][j] : 0));
        }
    }
}
void shift(int cur)
